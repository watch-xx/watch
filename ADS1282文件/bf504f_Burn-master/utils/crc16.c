...
/**
 * Добавляет контрольную суммы к сообщению
 * cmp - pointer to NA-01 command
 */
#pragma section("FLASH_code")
void add_crc16(u8 * cmd)
{
    u16 CRC;
    u8 i, len;

    /* Длина в первом сообщении */
    len = cmd[0] + 3;
    cmd[len - 2] = cmd[len - 1] = 0;

    for (i = 0, CRC = 0; i < len; ++i) {
	CRC = ((CRC << 8) + cmd[i]) ^ Crc16Table[(CRC >> 8) & 255];
    }

    cmd[len - 1] = (unsigned char) (CRC & 255);
    cmd[len - 2] = (unsigned char) ((CRC >> 8) & 255);
}


/**
 * Расчитывает CRC на приеме или передаче
 */
#pragma section("FLASH_code")
u16 get_crc16_table(u8 num)
{
